...
| `REQFILL sub_821867A0` | F-173: one line per call to the guest's streamer **request registrar**, registers-only (`src/gpu_device.cpp:8403`-ff, cap `n <= 24 || n % 2000`). It has exactly ONE caller (`sub_82187150+0x1A8`) and fires **twice per boot**, returning slot ids 3 and 4 with `r5 = 0` on both - `r5` is the only non-pointer argument, so it is where the inflate byte count should arrive and does not. Keep this hook register-only: a census in this subsystem once AV'd its consumer by reading guest memory (`ENQ sub_821BCB10` note)

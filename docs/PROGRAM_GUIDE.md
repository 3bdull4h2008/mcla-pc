...
| `SLOT-RECYCLE` / `SLOT-RECYCLE-SHARED` | `src/gpu_device.cpp:11652` (`GuestSlotTableInsert`) | MITIGATION, load-bearing since F-135. The host registers its served bodies in the guest's 15-entry device slot table at `0x82860740` and had no release path, so from the 16th body on `MakeMemoryStream` fell back to handing the guest a **pointer** as its handle — which the guest's own `GetSize`/`Seek` contract rejects with -1, and `sub_821BE8D8` then allocates and reads that -1 (F-134's 4 GB storm). First branch recycles a slot the guest read to its last byte (fires ~1× — the registry path never advances `pos`); second branch reuses the index this same host path handed out last, which is sound only because every `MakeMemoryStream` stream already shares the one static object `kMemStreamSlot` | 1 / 24 in `w147` |

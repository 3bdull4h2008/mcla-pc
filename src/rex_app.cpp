...
// Define gpu_plugin cvar storage without registering the FlagRegistrar at static init time.
// We override config_.gpu_plugin in OnPreSetup, so the cvar itself is unused.
std::string& FLAGS_gpu_plugin_storage_() {
...
bool ReXApp::OnCloseRequested(ui::UIEvent& e) {
  (void)e;
  return false; // Suppress window close for testing
}

...
// Phase 5 pipeline-state key + in-memory pipeline cache.
//
...
// unordered_map. The cache is intentionally a placeholder container today:
// D3D12 PSO objects live in D3D12Backend; this module owns key computation and
// a bounded, key->handle map the backend will fill as it creates PSOs.
...
// Bounded in-memory pipeline cache: key -> opaque backend handle (uint64_t).
class PipelineCache {
public:
    explicit PipelineCache(std::size_t maxEntries = 4096);

    // Insert or replace the handle for `key`. Returns the stored handle.
    uint64_t Insert(const PipelineKey& key, uint64_t handle);
    // Returns true + fills `handle` if the key is present.
    bool Find(const PipelineKey& key, uint64_t& handle) const;
    std::size_t Size() const { return map_.size(); }
    void Clear();

private:
    std::size_t maxEntries_;
    // insertion-order vector for simple FIFO eviction
    std::vector<PipelineKey> order_;
    std::unordered_map<PipelineKey, uint64_t, PipelineKeyHash> map_;
};

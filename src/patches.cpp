...
PPC_FUNC(sub_821C2AB8)
{
    ...
    if (n <= 16 || n % 1024 == 0)
    {
        MCLA_LOG_WARN("POOL-ROUTER #{} r3={:08X} r4={:08X} lr={:08X}",
            n, ctx.r3.u32, ctx.r4.u32, lr_val);
    }
}
...
// The authoritative decode lives in .clinerules/memory/ and docs/BOOT_HANDOFF.md.
...
// SESSION 37: entry-time fast-path capture counters (see PPC_FUNC(sub_821C29A0)).
...
PPC_FUNC(sub_821C29A0)
{
    ...
    if (ret == 0)
    {
        ...
        MCLA_LOG_WARN(
            ...
            oomIdx, self, sizeEntry, alignEntry, callerLr,
            preCap, preCarved, preFree,
            postCap, postCarved, postFree,
            b15Count, b15Sum, (flagsWord & 0xFFu));
    }
    ...
    if (n <= 32 || n % 2000 == 0)
    {
        ...
        MCLA_LOG_WARN(
            "OOM-CENSUS #{} self={:08X} req(r4)={:08X} r5={:08X} cap={:08X} free={:08X} "
...
PPC_FUNC(sub_821C1BB0)
{
    ...
    MCLA_LOG_WARN(
        ...
        "walked={} walkedsum={:08X} lr={:08X} blocks=[{:08X}/{:08X} {:08X}/{:08X} "
        ...
        firstAddr[0], firstSize[0], firstAddr[1], firstSize[1],
        firstAddr[2], firstSize[2], firstAddr[3], firstSize[3],
...
thread_local uint32_t t_lastSlabHead = 0; // head value after last pop on this thread
thread_local uint32_t t_lastSlabAddr = 0; // slab addr of last pop on this thread
#endif
} // namespace
...
PPC_FUNC(sub_821DE9D8)
{
                    ...
                    if (slabCount > 1 && slabHead != 0)
                    {
                        ...
                        if (chainLen != slabCount)
                        {
                            MCLA_LOG_WARN(
                                ...
                                checkIdx, curSlab, slabCount, chainLen,
                                slabHead,
                        ...
                        else
                        {
                            MCLA_LOG_WARN(
                                ...
                                checkIdx, curSlab, slabCount, chainLen,
                                slabHead, OomCensusReadU32(slabHead + 0));
                        }
                    }
    ...
    // Root cause: game code writes >16 bytes to 16-byte pool allocations, overflowing
    // into the adjacent element's freelist next pointer (at node+0 of the next element).
    // Since elements are packed at elemStride intervals, the overflow corrupts the chain.
    //
    // Strategy: Walk ALL slabs for this classHead.  When a slab's freelist head's next
    //   pointer is outside the slab data area, the chain is broken.  Attempt to scan
    //   all elements for 0xDD free pattern; if that fails (game overwrites freed memory),
    //   reset count+head to 0.  The allocator handles count=0 by creating fresh slabs.
    //   This loses ~632 free elements per corrupted slab but prevents OOM.
    {
            ...
            while (curSlab49 != 0)
            {
                ...
                if (cnt49 > 0 && hd49 != 0)
                {
                        ...
                        && (hdNext49 < curSlab49 + 32 || hdNext49 >= curSlab49 + 16384))
                    {
                        ...
                        MCLA_LOG_WARN(
                            "POOL-CHAIN-REBUILT slab={:08X} es={} cap={} "
    ...
    // classHead+4 layout (big-endian): halfword at +4 = elem stride, halfword at +6 = capacity
    // 32-bit read at +4: upper16 = lhz(+4) = elem stride, lower16 = lhz(+6) = capacity
    const uint32_t preHeadWord = OomCensusReadU32(classHead + 4);
    const uint32_t preElemsize = preHeadWord >> 16; // lhz(classHead+4) = element stride
    ...
    if (watchElemsize4)
    {
        ...
        if (preSlab != 0)
        {
            ...
            {
                MCLA_LOG_WARN(
                    "WRITE-WATCH HEAD-RESET slab={:08X} lastHead={:08X} "
    ...
    if (ctx.r3.u32 != 0)
    {
        ...
        if (a <= 16 || a % 4096 == 0)
        {
            ...
            MCLA_LOG_WARN(
                "POOL-ALLOC #{} classHead={:08X} node={:08X} slab={:08X} "
        ...
        // are consistent. Log every pop for the first 512, then only desyncs.
        {
            ...
            if ((hw >> 16) == 16 && (hw & 0xFFFF) == 1018 && headAfter != 0)
            {
                ...
                if (headNext != 0 && (headNext < slab + 32 || headNext >= slab + 16384))
                {
                    MCLA_LOG_WARN(
                        "POOL-CORRUPT-NEXT #{} node={:08X} slab={:08X} "
                        "headAfter={:08X} headNext={:08X} OUT-OF-SLAB lr={:08X} tid={:08X}",
                        k, node, slab, headAfter, headNext,
                        static_cast<uint32_t>(ctx.lr),
                        static_cast<uint32_t>(GetCurrentThreadId()));
                }
                // Also check: head valid but next==0 while count>1 means chain truncated
                if (headNext == 0 && countAfter > 1)
                {
                    MCLA_LOG_WARN(
                        "POOL-CHAIN-TRUNC #{} node={:08X} slab={:08X} "
            ...
            if (k <= 512 || desync)
            {
                MCLA_LOG_WARN(
                    ...
                    "tid={:08X}{}",
                    k, node, slab, countAfter, headAfter,
                    preAllocSlab, preAllocCount, preAllocHead,
                    static_cast<uint32_t>(ctx.lr),
                    static_cast<uint32_t>(GetCurrentThreadId()),
                    desync ? " DESYNC" : "");
            }
        ...
        if (watchElemsize4 && preSlab != 0 && preSlab == slab)
        {
            ...
            {
                MCLA_LOG_WARN(
                    "WRITE-WATCH NO-ADVANCE #{} node={:08X} slab={:08X} "
            ...
            if (nextAfter != 0 && (nextAfter < slab + 32 || nextAfter >= slab + 16384))
            {
                MCLA_LOG_WARN(
                    "WRITE-WATCH CORRUPT-NEXT #{} node={:08X} slab={:08X} "
                    "nextAfter={:08X} OUT-OF-SLAB lr={:08X} tid={:08X}",
                    k, node, slab, nextAfter,
                    static_cast<uint32_t>(ctx.lr),
                    static_cast<uint32_t>(GetCurrentThreadId()));
            }
        ...
        {
            MCLA_LOG_WARN(
                "POOL-POP #{} node={:08X} slab={:08X} countAfter={} "
    ...
    // SESSION 37: log EVERY return-0 (no sampling) for slab corruption root-cause.
    // classHead+4 layout: upper16 = lhz(+4) = elem stride, lower16 = lhz(+6) = capacity
    const uint32_t headWord = OomCensusReadU32(classHead + 4);
    ...
    // Slab layout: +0=next-newer, +4=prev-older, +8=count, +12=freelist, +16=owner
    // For each slab with count!=0, also walk the freelist to get chain length.
    uint32_t totalSlabs = 0, totalFree = 0, slabsWithFree = 0;
    uint32_t firstNonZeroSlab = 0, firstNonZeroCount = 0, firstNonZeroFreelist = 0, firstNonZeroChainLen = 0;
    ...
    while (curSlab != 0 && totalSlabs < 32)
    {
        ...
        MCLA_LOG_WARN(
            ...
            failIdx, totalSlabs, curSlab, sNext, sPrev, sCount, sFree, sOwner, chainLen);
    ...
    MCLA_LOG_WARN(
        "POOL-CENSUS FAIL#{} {} classHead={:08X} heap={:08X} elemsize={} capacity={}: "
        "headSlab={:08X} totalSlabs={} slabsWithFree={} totalFree={} "
        "preSlab={:08X} preCount={} preHead={:08X} lr={:08X}",
        failIdx, refilled ? "REFILLFAIL" : "NOREFILL", classHead, heap,
        elemsize, nodesPerSlab, headSlab, totalSlabs, slabsWithFree, totalFree,
        preAllocSlab, preAllocCount, preAllocHead,
        static_cast<uint32_t>(ctx.lr));
    ...
    {
        MCLA_LOG_WARN(
            "POOL-CENSUS FAIL#{} FIRST-NONZERO slab={:08X} count={} freelist={:08X} "
            "chainLen={} countMatch={}",
            failIdx, firstNonZeroSlab, firstNonZeroCount, firstNonZeroFreelist,
            firstNonZeroChainLen, firstNonZeroChainLen == firstNonZeroCount ? "YES" : "NO");
    }
    ...
    {
        MCLA_LOG_WARN(
            "POOL-CENSUS SLAB-CHANGED classHead={:08X} preSlab={:08X} "
            "postSlab={:08X} preCount={} preHead={:08X}",
            classHead, preAllocSlab, postAllocSlab, preAllocCount, preAllocHead);
    }
    else if (preAllocSlab != 0 && preAllocCount == 0 && firstNonZeroCount > 0)
    {
        MCLA_LOG_WARN(
            "POOL-CENSUS COUNT-JUMP classHead={:08X} slab={:08X} "
            "preCount=0 postCount={} preHead={:08X} postHead={:08X}",
            classHead, preAllocSlab, firstNonZeroCount, preAllocHead, firstNonZeroFreelist);
    }
}
...
PPC_FUNC(sub_821DE908)
{
    ...
    //   backChain+8 = saved LR (game caller's return address, security cookie pattern)
    //   backChain+36 = r29 (heap)
    //   backChain+40 = r28 (node passed to sub_821C09C8)
    ...
    // If freelist is already 0 before the free, the slab is pre-corrupted.
    {
        ...
        {
            MCLA_LOG_WARN(
                "POOL-FREE-PRE-DESINCRONIZED slab={:08X} node={:08X} "
                "countBefore={} freeHeadBefore={:08X} lr={:08X} tid={:08X}",
                slab, node, countBefore, freeHeadBefore,
                caller_lr,
                static_cast<uint32_t>(GetCurrentThreadId()));
        }
    }
    ...
    {
        MCLA_LOG_WARN(
            "POOL-FREE-FREELIST-ZERO slab={:08X} node={:08X} elemsize={} "
            "countAfter={} freeHeadBefore={:08X} freeHeadAfter={:08X} "
            "lr={:08X} tid={:08X}",
            slab, node, elemsize, countAfter,
            freeHeadBefore, freeHeadAfter,
            caller_lr,
            static_cast<uint32_t>(GetCurrentThreadId()));
    }
    ...
    {
        MCLA_LOG_WARN(
            "POOL-FREE-HEAD-MISMATCH slab={:08X} node={:08X} elemsize={} "
            "countAfter={} freeHeadBefore={:08X} freeHeadAfter={:08X} "
            "lr={:08X} tid={:08X}",
            slab, node, elemsize, countAfter,
            freeHeadBefore, freeHeadAfter,
    ...
    if (a <= 128 || a % 256 == 0)
    {
        ...
        MCLA_LOG_WARN(
            ...
            "r30={:08X} r31={:08X}",
            a, slab, node, elemsize, nodesPerSlab, countBefore, countAfter,
            off, freeHeadBefore, freeHeadAfter,
            savedLR, callerLR, callerBC,
            callerR28, callerR29,
            savedR30, savedR31);
    }
}
...
PPC_FUNC(sub_821C09C8)
{
    ...
    if (n <= 16 || n % 2048 == 0)
    {
        ...
        MCLA_LOG_WARN(
            ...
            n, heap, node, lr, sp,
            backChain, c0, c1, c2, c3,
            static_cast<uint32_t>(ctx.r5.u32),
            static_cast<uint32_t>(ctx.r6.u32),
            static_cast<uint32_t>(ctx.r7.u32),
            static_cast<uint32_t>(ctx.r8.u32),
            static_cast<uint32_t>(ctx.r9.u32),
            static_cast<uint32_t>(ctx.r10.u32),
...
// ---------------------------------------------------------------------------
// SESSION 50/51: Capture-only census on sub_8218CC70 — config dispatch table dump.
// The dispatch formula (from ppc_recomp.10.cpp:17326):
//   r3 = handler index
...
PPC_FUNC(sub_82177EB0)
{
    ...
    const bool anyRegInCrashRange = (r3 | r4 | r5 | r6 | r7 | r8 | r9 | r10) >= 0xC9000000u
                                 && (r3 | r4 | r5 | r6 | r7 | r8 | r9 | r10) < 0xCA000000u;

    if (r3LooksLikePointer || anyRegInCrashRange)
    {
        MCLA_LOG_WARN(
            "SUB82177EB0: lr={:08X} r3={:08X} r4={:08X} r5={:08X} "
...
PPC_FUNC(sub_8218CC70)
{
    ...
    // This ensures we see EVERY entry even if crash kills spdlog's async thread
    {
        static FILE* s_entryLog = nullptr;
        if (!s_entryLog) s_entryLog = fopen("cache/dispatch_entry.log", "a");
        if (s_entryLog) {
            const uint32_t r3v = ctx.r3.u32;
            const uint32_t lr = static_cast<uint32_t>(ctx.lr);
            fprintf(s_entryLog, "ENTRY r3=%08X lr=%08X\n", r3v, lr);
            fflush(s_entryLog);
        }
    }
    ...
    constexpr uint32_t kMaxTableIdx = 46u;
    const bool inBounds = handlerIdx < kMaxTableIdx;
    const uint32_t slotAddr = kTableBase + handlerIdx * 4;
    const uint32_t rawOffset = inBounds ? OomCensusReadU32(slotAddr) : 0xDEADBEEFu;
    ...
    else
    {
        ...
        if (targetFn == 0)
            validity = "NULL";
        else if (targetFn >= 0x82130000u && targetFn < 0x83000000u)
            validity = "VALID_CODE";
        else if (targetFn >= 0x82830000u && targetFn < 0x83000000u)
            validity = "VALID_DATA";
    ...
    if (n == 1)
    {
        MCLA_LOG_WARN("CONFIG-DISPATCH: === TABLE DUMP (table=0x827D6010 global=0x82839254) ===");
        for (uint32_t i = 0; i < 64; i++)
        {
            const uint32_t addr = kTableBase + i * 4;
            const uint32_t val = OomCensusReadU32(addr);
            if (val == 0 && i > 40) break;
            MCLA_LOG_WARN("CONFIG-DISPATCH: TBL[{:2}] @ {:08X} = {:08X}{}", i, addr, val,
                (val == 0xFFFFFFFFu) ? " (SENTINEL)" : "");
        }
        for (uint32_t g = 0; g < 8; g++)
        {
            const uint32_t gaddr = kGlobalBase + g * 4;
            MCLA_LOG_WARN("CONFIG-DISPATCH: GLOBAL @ {:08X} = {:08X}", gaddr, OomCensusReadU32(gaddr));
        }
    }

    // OOB: always log with full register dump — flush immediately (crash imminent)
    if (!inBounds)
    {
        ...
        {
            MCLA_LOG_WARN(
                "CONFIG-DISPATCH-OOB #{} idx={:08X} lr={:08X} | "
                "r3={:08X} r4={:08X} r5={:08X} r6={:08X} r7={:08X} r8={:08X} r9={:08X} r10={:08X}",
                oobN, handlerIdx, callerLr,
    ...
    // Log valid calls (first 64 + every 1024th)
    if (inBounds && (n <= 64 || n % 1024 == 0))
    {
        MCLA_LOG_WARN(
            "CONFIG-DISPATCH #{} idx={:08X} lr={:08X} | slot={:08X} rawOff={:08X} "
            "basePtr={:08X} struct={:08X} targetFn={:08X} [{}]",
            n, handlerIdx, callerLr,
            slotAddr, rawOffset, basePtr, structAddr, targetFn, validity);
        spdlog::default_logger()->flush();
    }

    // Call the original unchanged

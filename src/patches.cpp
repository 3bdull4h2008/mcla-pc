...
void hk_KeWaitForSingleObject(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_XamInputGetState(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_NtCreateFile(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_NtReadFile(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_NtQueryInformationFile(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_NtClose(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_82130690(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_82130770(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_82130850(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_82130930(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_82130A18(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_822A3998(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_82554E20(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_sub_82554590(mcla::native::PPCContext& ctx, uint8_t* base);
void hk_GpuKick(mcla::native::PPCContext& ctx, uint8_t* base);
static PPCFunc* g_orig_sub_82554E20 = nullptr;
static PPCFunc* g_orig_VdSwap_observer = nullptr;

REX_FUNC(hk_vdswap_observer) {
    static std::atomic<uint64_t> s_vdswap_count{0};
    uint64_t c = s_vdswap_count.fetch_add(1) + 1;
    if (c <= 5 || c % 300 == 0) {
        REXLOG_INFO("VdSwapObserver[{}] obj=0x{:08X} swap_info=0x{:08X}",
...
static void MclaPatchSdkExecutePacketType3Overflow() {
    ...
    if (!mod) {
        REXLOG_ERROR("SDK overflow patch: rexgpu-xenos.dll not loaded");
        return;
    }
    uint8_t* base = reinterpret_cast<uint8_t*>(mod);
    uint8_t* sig = base + kEpt3SigRva;
    if (memcmp(sig, kEpt3Signature, kEpt3SignatureLen) != 0) {
        REXLOG_ERROR("SDK overflow patch: signature mismatch at 0x1C25A; "
    ...
    if (!VirtualProtect(target, sizeof(patch), PAGE_EXECUTE_READWRITE, &old)) {
        REXLOG_ERROR("SDK overflow patch: VirtualProtect failed ({})", GetLastError());
        return;
    }
    ...
    REXLOG_WARN("SDK overflow patch applied at 0x1C262: ExecutePacketType3 "
                "overflow now drains ring + returns true (non-fatal)");
}

REXCVAR_DEFINE_STRING(mcla_patch_groups, "all", "MCLA Renderer",
                      "Which hook groups to install: all|none|sdk,guest,native");
bool BisectGroupEnabled(std::string_view group) {
    const std::string_view groups = REXCVAR_GET(mcla_patch_groups);
...
void mcla_ApplyPatches(mcla::App::FunctionDispatcher* dispatcher) {
    ...
    if (mode == "compat") {
        REXLOG_WARN("mcla_ApplyPatches: COMPAT mode - only 0x82554080 stub "
    ...
    if (!BisectGroupEnabled("guest") && !BisectGroupEnabled("ke") &&
        ...
        REXLOG_WARN("mcla_patch_groups: no guest sub-group enabled - all guest "
                    "hooks (KeWait/FS/input/GPU-kick/DPC/PressStart) skipped");
        REXLOG_INFO("MCLA patches applied (stub only)");
        return;
    }
    ...
    if (mode != "compat" && !BisectGroupEnabled("native")) {
        ...
        g_orig_VdSwap_observer = mcla_DetourImportThunk(
            ...
            hk_vdswap_observer);
        if (g_orig_VdSwap_observer) {
            REXLOG_INFO("VdSwap observer installed (passthrough, legacy mode)");
        } else {
            REXLOG_ERROR("VdSwap observer: thunk detour FAILED");
        }
    }
...
REX_FUNC(hk_GpuKick) {
    ...
    if (mgr) {
        ...
        if (!s_kick_diag_done) {
            ...
            REXLOG_WARN("GpuKick diag: mgr=0x{:08X} +48(write)=0x{:08X} +52(limit)=0x{:08X} +56={:08X}",
                        mgr, d48, d52, d56);
        }
...
REX_FUNC(hk_XamInputGetState) {
    ...
    if (buf >= 0x10000000) {
        if (StartPulseActive()) {
            REX_STORE_U16(buf + 4, 0x0010);
        } else {
            REX_STORE_U16(buf + 4, 0x0000);
        }
        if (callCount <= 5) {
...
REX_FUNC(hk_sub_822A3998) {
    ...
    REXLOG_INFO("hk_sub_822A3998 called");
}

REX_FUNC(hk_sub_82554590) {
    // Placeholder
    REXLOG_INFO("hk_sub_82554590 called");
}
...
static void ReinitGpuBackend(mcla::native::PPCContext& ctx, uint8_t* base) {
    ...
    if (ring_buf) {
        ...
        auto saved_lr = ctx.lr;
        ctx.r3.u64 = ring_buf;
        REX_CALL_INDIRECT_FUNC(0x827BD5F4);
        ctx.r4.u64 = 12;
        REX_CALL_INDIRECT_FUNC(0x827BD6B4);
        ctx.lr = saved_lr;
    }
    ...
    if (sub_ctx) {
        ...
        REX_CALL_INDIRECT_FUNC(0x827BD5F4);
        ctx.r4.u64 = 19;
        REX_CALL_INDIRECT_FUNC(0x827BD6A4);
        ctx.lr = saved_lr;
    }

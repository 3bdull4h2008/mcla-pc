...
static uint8_t* g_virtual_membase = nullptr;  // guest→host translation base
...
// The native XamInputGetState hook — overwrites the DLL function entry to
...
// ── Native function detour helpers ───────────────────────────────────────────
...
// NOTE: We're NOT calling through to the original — returning 0 with
...
extern "C" DWORD __fastcall hk_native_XamInputGetState(DWORD port, void* state) {
    ...
    if (state) {
        ...
        *buttons |= 0x0010;   // XINPUT_GAMEPAD_START
        if (callCount <= 5) {
            REXLOG_INFO("hk_native_XamInputGetState[{}]: injected Start (old=0x{:04X})", 
                        callCount, old);
        }
    }
...
static PPCFunc* DetourImportThunk(uint8_t* thunk, PPCFunc* hook) {
...
// matching the format the game's timer handlers expect (fcfid→frsp→fmuls chain).
...
// Non-null → timers are armed and sub_821BE860 is used as handler.
// Null     → timers are disarmed and sub_823D9750 is used instead.
...
// ── Kernel Timer DPC hooks ──────────────────────────────────────────────────
// All four follow the same structure:
//   delta = REX_QUERY_TIMEBASE() - stored_timebase  →  (float)delta * freq
...
void mcla_ApplyPatches(rex::runtime::FunctionDispatcher* dispatcher) {
    ...
    g_original_KeWait = DetourImportThunk(
    ...
    // Hook XamInputGetState — same approach as KeWait: overwrite the
    ...
    g_original_XamInputGetState = DetourImportThunk(
    ...
    if (!g_original_XamInputGetState) {
        ...
        // Capture membase for guest→host address translation
    ...
    // CPU speed — when VSync is removed the guest spins faster and timebase
    ...
    // the native input-check entirely.
    {
        ...
        // in the function table — REX_CALL_INDIRECT_FUNC would try to JIT
        ...
        if (!dispatcher->SetFunction(g_press_start_shim_thunk, hk_press_start_shim)) {
            REXLOG_ERROR("Press Start shim: SetFunction(0x{:08X}) FAILED — "
    ...
    // Screen manager hook — log first few calls for debugging
...
static void ReinitGpuBackend(PPCContext& ctx, uint8_t* base) {
    ...
    // 1. VdInitializeEngines — reset GPU backend internal state.
    ...
    // 2. VdSetGraphicsInterruptCallback — re-register the game's VSync
    ...
    if (ring_buf) {
        ...
        REX_CALL_INDIRECT_FUNC(0x827BD5F4); // MmGetPhysicalAddress → r3 = phys addr
...
// Hook: KeWaitForSingleObject — skip the display-sync semaphore wait.
...
REX_FUNC(hk_KeWaitForSingleObject) {
    ...
    //   0x828CD9AC — polled at very high frequency (dozens/ms), this IS the
    //                display sync semaphore (not 0x8234D9AC as originally
    //                guessed — the base page is 0x828C, not 0x8234).
    //   0x40004D7C — polled at ~30 ms intervals on the main thread (must NOT
...
// Hook: XamInputGetState — inject a Start button press so the game
...
// path separately, but logs show it never fires — the handler at
...
REX_FUNC(hk_XamInputGetState) {
    ...
    if (callCount <= 10) {
        REXLOG_INFO("XamInputGetState[{}]: port={} buf=0x{:08X}", callCount, port, buf);
    }

    // Always inject Start press into every XamInputGetState output buffer
    if (buf >= 0x10000000) {
        REX_STORE_U16(buf + 4, 0x0010);  // XINPUT_GAMEPAD_START
        if (callCount <= 5) {
            REXLOG_INFO("XamInputGetState[{}]: injected Start into buf=0x{:08X}", callCount, buf);
        }
    }
...
// Hook: sub_822A3998 — input device getter.
...
REX_FUNC(hk_sub_822A3998) {
    ...
    if (port == 0) {
        ...
        if (!s_fakeInputDeviceAddr) {
            ...
            // 0x828CD9AC is 4 bytes — we write 12 bytes into the
            // following padding.
            for (int i = 0; i < 12; i++) {
...
// Hook: sub_82554E20 — screen manager main entry point.
...
REX_FUNC(hk_sub_82554E20) {
    ...
    if (screenObj == 0xBD3AFCB0) {
        ...
        // Chain to original — it will execute bytecodes and call our shim
        if (g_orig_sub_82554E20) {
...
// Shim: Press Start handler — sets flag=2 to trigger transition,

...
std::atomic<uint32_t> g_primarySlot{kInvalidSlot}; // doorbell target (most recent init/WB)
...
std::atomic<uint32_t> g_pendingRingWptr{0xFFFFFFFFu}; // doorbell deferred to vblank
...
RingState* RingOwningCursorVA(uint32_t va)
{
    return RingContainingVA(va);
}
...
// ---------------------------------------------------------------------------
// Guest publication contract (IDA-proven 2026-08-26, .research/mcla_rex_contract):
...
void CpAdvanceGuestPublication(uint32_t windowsCompleted)
{
    ...
    ForEachDevice([&](uint32_t dev) {
        ...
        if (next != st->count)
        {
            ...
            if (n <= 24 || (n % 500) == 0)
            {
                MCLA_LOG_INFO("CP: GUEST-PUB #{} dev={:08X} pub {}->{} put={}",
                              n, dev, prev, next, put);
            }
        }
    });
...
void DrainIndirectBuffer(uint32_t byteAddr, uint32_t dwordCount, int depth)
{
    ...
    while (p < limited)
    {
        ...
        {
            uint32_t hdrProbe = 0;
            if (mcla::kernel::GuestMemoryHeap::Instance().ReadU32BE(byteAddr + p * 4u, &hdrProbe))
            {
    ...
    if (DeferredConsumeEnabled() && p != 0)
    {
        ...
        if (nIb <= 12 || (nIb % 200) == 0)
        {
            MCLA_LOG_INFO("CP: IB-CREDIT #{} base={:08X} credited={} dwords",
                          nIb, byteAddr, p);
        }
    }
}
...
void DrainRing(RingState& ring, uint32_t wptr, const char* source)
{
    ...
    while (rptr != wptr && guard--)
    {
        ...
        {
            MCLA_LOG_ERROR("CP: RING {} desync at rptr={} header={:08X} - dropping to wptr {:04X}",
...
uint32_t CpScbToken()
{
    return g_scbToken.load(std::memory_order_relaxed);
}
...
void CpInitializeRingBuffer(uint32_t physAddr, uint32_t sizeLog2)
{
    ...
    if (slot < 0)
    {
        ...
        for (auto& other : g_rings)
        {
            if (&other == ring || !other.initialized.load(std::memory_order_acquire))
                continue;
            const bool overlap = physAddr < other.EndByte() &&
                                 other.baseGuestVA < ring->EndByte();
            if (overlap)
            {
                MCLA_LOG_WARN("CP: RING {} [{:08X},{:08X}) overlaps RING {} "
                              "[{:08X},{:08X}) - guest re-used ring memory with "
                              "a different identity",
                              ring->id, ring->baseGuestVA, ring->EndByte(),
                              other.id, other.baseGuestVA, other.EndByte());
            }
        }
    ...
    else
    {
        ...
        const uint32_t lastWptr = ring->lastDoorbellWptr.load(std::memory_order_acquire);
        const bool quiescent =
            lastWptr == 0xFFFFFFFFu || rptr == lastWptr;
        g_primarySlot.store(static_cast<uint32_t>(slot), std::memory_order_release);
        if (quiescent)
        {
            ring->rptrIndex.store(0, std::memory_order_relaxed);
...
void CpEnableRPtrWriteBack(uint32_t rptrWritebackAddr, uint32_t blockSizeLog2)
{
    ...
    {
        MCLA_LOG_INFO("CP: RING {} rptr writeback raw={:08X} (phys) -> publish @ {:08X} (kernel VA)",
...
void CpVblankDrainToWptr()
{
    ...
    {
        uint32_t subctx = 0;
        if (probeDev != 0 && mem.ReadU32BE(probeDev + 10896, &subctx) && subctx != 0)
    ...
    if (n <= 24 || (n % 500) == 0)
    {
        MCLA_LOG_INFO("CP: VDRAIN #{} RING {} base={:08X} cap={} dev={:08X} "
                      ...
                      "wb={:08X} pub {:X}->{}",
                      n, ring->id, ring->baseGuestVA, cap, probeDev, wptrVA,
                      wptr, oldRptr, newRptr, consumed,
...
bool CpMmioWrite(uint32_t guestAddr, uint32_t value)
{
    ...
    if (offset == kWptrByteOffset)
    {
        ...
        if (n <= 16 || (n % 500) == 0)
        {
            if (ring)
            {
                MCLA_LOG_INFO("CP: doorbell RING {} base={:08X} cap={} wptr={:08X} (rptr={})",
                              ring->id, ring->baseGuestVA, ring->capDwords, value,
                              rptrBefore);
            }
            else
            {
            ...
            if (ring && acquired)
            {
                ...
                if (n <= 16 || (n % 500) == 0)
                {
                    MCLA_LOG_INFO("CP: RING {} DRAIN src=doorbell base={:08X} cap={} "
                                  ...
                                  "published={} lr={:08X}",
                                  ring->id, ring->baseGuestVA, ring->capDwords,
                                  startRptr, endRptr, value, consumed,
                                  ring->writebackVA.load(std::memory_order_relaxed),
                                  consumed != 0, callerLr);
                }
                ring->draining.store(false, std::memory_order_release);

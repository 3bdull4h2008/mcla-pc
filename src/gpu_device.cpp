...
// Mutex for protecting g_lastDrawV2 access
...
std::mutex& mcla_gpu_GetDrawMutex()
{
    return g_drawMutex;
}
...
PPC_FUNC(sub_82413660)
{
    ...
    if (n <= 10 || (n % 1000) == 0)
    {
        MCLA_LOG_INFO("DRAW-SEAM sub_82413660 #{} dev={:08X} r4={:08X} r5={:08X} r6={:08X} lr={:08X}",
...
PPC_FUNC(sub_82419718)
{
    ...
    if (n <= 12 || (n % 2000) == 0)
        MCLA_LOG_INFO("FLIP sub_82419718 #{} dev={:08X} cb={:08X} q[0..2]={:08X}/{:08X}/{:08X}",
                      n, dev, cb, q0, q1, q2);
    // Increment frame counter on each flip for draw-flip correlation

...
GuestThreadContext::GuestThreadContext(uint32_t cpuNumber)
{
    ...
    *(uint32_t*)thread = ByteSwap(mcla::kernel::g_memory.MapVirtual(thread + PCR_SIZE)); // tls pointer
    *(uint32_t*)(thread + 0x100) = ByteSwap(mcla::kernel::g_memory.MapVirtual(thread + PCR_SIZE + TLS_SIZE)); // teb pointer
    ...
    *(uint32_t*)(thread + PCR_SIZE + TLS_SIZE + 0x14C) = ByteSwap(GuestThread::GetCurrentThreadId()); // thread id
    ...
    ppcContext.r1.u64 = mcla::kernel::g_memory.MapVirtual(thread + PCR_SIZE + TLS_SIZE + TEB_SIZE + STACK_SIZE); // stack pointer
    ppcContext.r13.u64 = mcla::kernel::g_memory.MapVirtual(thread);
...
{
    auto* thread = (char*)mcla::kernel::g_memory.Translate(GetPPCContext()->r13.u32);
...
{
#ifdef _WIN32
    GuestThread::SetThreadName(threadId, (const char*)mcla::kernel::g_memory.Translate(ByteSwap(*name)));
#endif
}

...
struct ArgTranslator
{
    ...
    constexpr static std::enable_if_t<!std::is_pointer_v<T>, void> SetValue(PPCContext& ctx, uint8_t* base, size_t idx, T value) noexcept
    {
        ...
        {
            SetIntegerArgumentValue(ctx, base, idx, mcla::kernel::g_memory.MapVirtual(value));
        }
        else
        {
    ...
    constexpr static std::enable_if_t<std::is_pointer_v<T>, void> SetValue(PPCContext& ctx, uint8_t* base, size_t idx, T value) noexcept
    {
        const auto v = mcla::kernel::g_memory.MapVirtual((void*)value);
        if (!v)
        {
            return;
        }
...
T GuestToHostFunction(const TFunction& func, TArgs&&... argv)
{
    ...
    _translate_args_to_guest(newCtx, mcla::kernel::g_memory.base, args);

    SetPPCContext(newCtx);

    if constexpr (std::is_function_v<TFunction>)
        func(newCtx, mcla::kernel::g_memory.base);
    else
        mcla::kernel::g_memory.FindFunction(func)(newCtx, mcla::kernel::g_memory.base);
    ...
    {
        return reinterpret_cast<T>((uint64_t)mcla::kernel::g_memory.Translate(newCtx.r3.u32));
    }

...
// Guest reads/writes route through the checked GuestMemoryView (endian-explicit
// BE access). The ABI header is copied verbatim into the generated TUs, so the
// helpers must live in guest_memory.h and link through guest_memory.cpp.
...
#ifndef PPC_LOAD_U8
#define PPC_LOAD_U8(x) mcla::native::ReadGuestU8((uint32_t)(x))
#endif

#ifndef PPC_LOAD_U16
#define PPC_LOAD_U16(x) mcla::native::ReadGuestU16((uint32_t)(x))
#endif

#ifndef PPC_LOAD_U32
#define PPC_LOAD_U32(x) mcla::native::ReadGuestU32((uint32_t)(x))
#endif

#ifndef PPC_LOAD_U64
#define PPC_LOAD_U64(x) mcla::native::ReadGuestU64((uint32_t)(x))
#endif
...
#ifndef PPC_STORE_U8
#define PPC_STORE_U8(x, y) mcla::native::WriteGuestU8((uint32_t)(x), (y))
#endif

#ifndef PPC_STORE_U16
#define PPC_STORE_U16(x, y) mcla::native::WriteGuestU16((uint32_t)(x), (y))
#endif

#ifndef PPC_STORE_U32
#define PPC_STORE_U32(x, y) mcla::native::WriteGuestU32((uint32_t)(x), (y))
#endif

#ifndef PPC_STORE_U64
#define PPC_STORE_U64(x, y) mcla::native::WriteGuestU64((uint32_t)(x), (y))
#endif
...
// Query guest timebase (replaces ReXGlue REX_QUERY_TIMEBASE / rex::chrono::Clock::QueryGuestTickCount)
#ifndef PPC_QUERY_TIMEBASE
#define PPC_QUERY_TIMEBASE() mcla::native::QueryGuestTimebase()
#endif
...
// Compatibility: unsigned variant of vctsxs (AVX-512 VCvtUPS2UQ / VCvtUPS2UDQ)
// Converts packed single-precision floats to packed unsigned 32-bit ints with saturation.
inline simde__m128i simde_mm_vctuxs(simde__m128 src1)
{
    simde__m128 xmm2 = simde_mm_cmpunord_ps(src1, src1);
    simde__m128i xmm0 = simde_mm_cvttps_epi32(src1);
    // For unsigned saturation: clamp negative values to 0, positive overflow to UINT_MAX
    simde__m128i xmm1 = simde_mm_cmplt_epi32(xmm0, simde_mm_setzero_si128());
    simde__m128i xmm2_u = simde_mm_cmpgt_epi32(xmm0, simde_mm_set1_epi32(INT_MAX));
    xmm0 = simde_mm_max_epi32(xmm0, simde_mm_setzero_si128());
    xmm0 = simde_mm_min_epi32(xmm0, simde_mm_set1_epi32(INT_MAX));
    // Note: full UINT_MAX saturation would require 64-bit intermediates; this clamps to INT_MAX range
    return simde_mm_andnot_si128(simde_mm_castps_si128(xmm2), simde_mm_castps_si128(xmm0));
}

...
Six sessions were lost to getting this wrong. Follow the steps **in order**; each one has a
...
```bash
cd "E:/mcla pc" && MSYS_NO_PATHCONV=1 MSYS2_ARG_CONV_EXCL='*' cmd.exe /c ninja_build.bat > build/<wave>.log 2>&1; echo "BUILD_RC=$?"
```
...
```bash
cd "E:/mcla pc" && timeout 120 ./build/mcla.exe > build/w38a.log 2> build/w38a_err.log; echo "BOOT_RC=$?"
```
